...
{
    // Ambil data homestay yang diklik beserta relasinya
    $homestay = Homestay::with(['tipe_kamars', 'tipe_kamars.kamars'])
                       ->findOrFail($id);
    
    // Ambil data homestays untuk bagian terkait (jika diperlukan)
    $homestays = Homestay::where('id', '!=', $id)
                        ->inRandomOrder()
                        ->limit(4)
                        ->get();
    
    return view('pemilik.homestays.detail', [
        'homestay' => $homestay,  // Data utama (singular)
        'homestays' => $homestays // Data terkait (plural)
    ]);
}
